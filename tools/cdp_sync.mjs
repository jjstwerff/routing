...
// (echo-free apply). NOTE: overwrites the developer's "_working" sketch.
...
const s1 = await t1.evaluate(`(async () => {
  ...
  routing.rough.setPoints([{ lat: 0.0, lon: -30.0 }, { lat: 0.01, lon: -30.0 }]);
  await sleep(2000);
...
const s2 = await t2.evaluate(`(async () => {
  ...
  let n = 0;
  for (let i = 0; i < 40 && n < 2; i++) { await sleep(250); n = (routing.roughPoints || []).length; }
...
await t1.evaluate(`(async () => {
  routing.rough.setPoints([{ lat: 0.0, lon: -30.0 }, { lat: 0.01, lon: -30.0 }, { lat: 0.02, lon: -30.0 }]);
  return JSON.stringify({ ok: true });
})()`);
...
const s3 = await t2.evaluate(`(async () => {
  const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  let n = 0;
  for (let i = 0; i < 120 && n < 3; i++) { await sleep(250); n = (routing.roughPoints || []).length; }
