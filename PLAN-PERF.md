...
| **18** ⏸ | **BLOCKED on @PLN108** — not designable yet | The blocker is neither expressiveness (17's error) nor the slice (18a's): `clone_for_worker()` **byte-copies every ACTIVE parent store per worker**, so par's cost tracks the **session's live heap** — even for a worker that touches none of it. **Measured** (`tools/par_copy_probe.loft`, filed as @PLN108's step 1): 0→122 MB of *unrelated* heap takes a fixed workload **2 → 205 ms**, and 1→16 threads takes it **36 → 178 ms** — *par gets 5× SLOWER with more threads.* Routing's match session exists to HOLD state (steps 6–8; RSS ~175 MB), so every dispatch would pay ~200 ms+ against a ~339 ms match: **the copy costs more than the work it parallelises.** @PLN108 (share read-only stores) removes exactly this; it cites routing as the consumer. **Re-design 18 only after it lands** — and re-run the probe against `LOFT_PAR_SHARE=1` first. | @PLN108 step 1 answered: NOT a rounding error | none (nothing to build) |
